...
check inputList hyps = checkHelper inputList 1 Map.empty Map.empty where
	checkHelper (expr : exprTail) !n prooved btoa | Map.member expr prooved = (Useless) : checkHelper exprTail (n + 1) prooved btoa
									  		      | numberAx expr /= -1 = (Axiom $ numberAx expr) : checkHelper exprTail (n + 1) (Map.insert expr n prooved) (updateBtoA expr btoa n)
